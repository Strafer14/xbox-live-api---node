...
fetchPreAuthData = async () => {
    ...
    if (!url_post || !ppft_re) {
        const post_vals = {
            ...
            , options = {
                uri: 'https://login.live.com' + '/oauth20_authorize.srf?' + post_vals_qs,
...
authenticate = async () => {
    ...
    if (cacheToken.value && cacheUhs.value && cacheNotAfter.value)
        return { token: cacheToken.value, uhs: cacheUhs.value, notAfter: cacheNotAfter.value };
    else {
...
xla.getClipsForGamer = async (gamertag) => {
    ...
    if (xuid < 0)
        return { "gameClips": [], "pagingInfo": { "continuationToken": null }, "noXuid": true };
...
xla.getDetailsForClip = async (gamertag, clipId) => {
    ...
    for (clip of clipData.gameClips)
        if (clip.gameClipId == clipId)
            return clip;

    if (continueToken)
        xla.getClipsForGamer(gamertag, continueToken);
    else {
        return;
    }
}
...
xla.getScreenshotsForGamer = async (gamertag) => {
    const xuid = await xla.getXuid(gamertag)
    if (xuid < 0) {
        return { "screenshots": [], "pagingInfo": { "continuationToken": null }, "noXuid": true };
    }
...
const internalRequest = async (host, uri) => {
    ...
    if (requestResults.statusCode != '200')
        console.log("Get Clips for Gamer XBL statusCode: ", requestResults.statusCode);
    let str = requestResults.body;
    try {
        str = JSON.parse(str);
    } catch (e) {
        return { "gameClips": [], "pagingInfo": { "continuationToken": null }, "jsonParseFail": true }
    }
    return str;
}
